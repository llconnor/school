...
template <class T> int Graph<T>::Build(FILE *infile, int source, int destination)
{
  ...
  //  infile >> size; //This gets the size of the file
  fread(getter, 1, 1, infile);
  size = getter[0];
  cout << "Size " << size << endl;
  vector<Alist<int> > v(size + 1);
  (*this).Lists = v;  int num = 1;
  int loopnum;
  while (num < size)
    {
      node = num;
      ...
      while(loopnum < size)
	{
	  ...
	  node = num;
	  neigh = loopnum;
	  //fread( &your_unsigned_char_variable, 1, 1, yourfileptr);
	  ...
	  if (dist > 0)
	    {
	      if(Lists[node].Addneighbor(neigh, dist, node) == -1)
		return -1;
	      else
		cout << "Node: " << node << "-" << dist << "-" << neigh << endl;
	    }
	  loopnum++;
	}
      num++;
...
template <class T> void Graph<T>::PrintFinal(int source,int destination, int p)
{
  ...
  //If the distance is 0 is infinity
