...
int init_sock( int mode )
{
  ...
  if (bind( sock, host_addr, host_addr_len )==-1)
    ...
    unsigned address )
{
  ...
  /* Send the packet */
  if (sendto( sock, p_data, p_length, 0, dest_addr, dest_addr_len )==-1)
...
sockaddr_in ) );
  ...
  datalen = recvfrom( sock, buf, buflen, 0, source_addr, &source_addr_len );
  if (datalen==-1)
    { perror( "recv_packet " ); exit( 1 ); }
