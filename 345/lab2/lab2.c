...
{  
  ...
  cout << "What server would you like to connect to?" << endl;
  cin >> servername;

  p_data = "GET"; //The data that we are sending
  len = 4; //The length of the message (including the /0 at the end)
  //  outfile.open("image.gif", ios::out);
  ofstream outfile("image.gif");

  sock = init_sock(CLIENT_MODE); //Initialize the port
  addr = get_addr_by_name(servername); //Get our servers address
...
//      cout << "addr" <<  addr << endl;
	...
	cout << "Error in Transmission of GET" << endl;
    }
while(1)
{
  sizerec = 271;
  errorint = recv_packet(data, &sizerec, 10);
  if(errorint == 2)
   cout << "Time Out" << endl;
  packtype = data[8];
  if(packtype == 0x81)
    {
      cout << "# of Bad Packets = " << numbadpackets << endl;
      outfile.close();
      return 0;
    }
  else if(packtype == 0x41)
    {
        packlength = data[11];

        if(check_crc(data) == 1)
      	{
//        cout << packlength << endl;
        for(int i = 12; i < packlength + 12; i++)
          outfile << data[i];
        }
        else
	  {
numbadpackets++;
    ...
    {
      cout << "Error with type, packet discarded" << endl;
      cout << "# of Bad Packets = " << numbadpackets << endl;
      return 0;
    }
}
